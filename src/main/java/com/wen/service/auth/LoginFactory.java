...
public class LoginFactory {
    ...
    // 缓存处理器映射关系
    private Map<LoginType, LoginHandler> handlerMap;
    ...
    public UserInfoResponse executeLogin(LoginRequest request) {
        ...
        LoginHandler handler = handlerMap.get(loginType);
        if (handler == null) {
            throw new BusinessException("暂不支持该登录方式：" + loginType);
        }

        log.info("执行登录，类型：{}", loginType);
     ...
     * 获取所有支持的登录方式
     */
    public List<LoginType> getSupportedTypes() {
        initHandlerMap();
